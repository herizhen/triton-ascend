...
>
- Theoretical time consumption for compute-related pipelines (Cube/Vector/Scalar, etc.) = Amount of data computed (Unit: Element) / Theoretical compute power. For example: The theoretical peak Vector compute power for float data type on a certain AI processor is 11.06 TOPS. To perform a single instruction computation on 32K float elements, the theoretical compute time is 32K / 11.06TOPS = 0.003us (calculated as 1K = 1000).
...
By analyzing the data in the figure, it can be found that `aiv_scalar_time(us)` and `aiv_scalar_ratio` differ significantly before and after optimization, indicating that the poor performance is due to a large number of scalar operations.
