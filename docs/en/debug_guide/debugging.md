...
This document is the **Triton-Ascend Debugging Guide**, designed for engineers involved in the adaptation and development of Triton with Ascend NPUs. It systematically introduces common debugging methods and tools used during the Triton-Ascend compilation and execution process.
...
During development, problems can generally be categorized as follows. The table below provides a quick guide for problem type identification and recommended primary debugging methods.

| Problem Type | Typical Manifestation/Description | Recommended Primary Debugging Method |
| :--- | :--- | :--- |
| **Precision Issue** | NPU execution results differ from the reference baseline (e.g., PyTorch or Triton CPU Interpreter). | 4. Interpreter Mode <br> 5.1 Print Debugging Methods |
...
| **Triton IR Adaptation** | `ttir.mlir` | `ttadapter.mlir` | Triton Backend adapted for Ascend | **Key Debugging Stage**. Converts TTIR to Adapter IR targeting the Ascend NPU backend. |
...
**The debugging focus of this guide** is on the second stage: the conversion process from `ttir.mlir` to `ttadapter.mlir`. This stage is the primary functionality of Triton-Ascend.
...
Disable Cache During Debugging: When debugging compilation issues, it is recommended to temporarily disable the cache to ensure a fresh compilation every time:
...
Even if caching is enabled, as long as `TRITON_DEBUG=1` is set, the system will regenerate dump files on each run (overwriting files in the same-named directory). However, if the cache is hit and compilation is skipped, the IR conversion may not be triggered, resulting in no new dumps. Therefore, it is recommended to also set the following during debugging:
...
|----------|----------|----------|----------|
| Cache Files | On each compilation execution | Generated when cache is missed | Clean periodically or when troubleshooting |
...
After running, the dump file path will be printed, defaulting to `~/.triton/dump`, displayed as follows:
...
- The function `@add_kernel` receives three pointer arguments (corresponding to the device memory addresses of input A, B, and output C) and one integer argument `n` representing the vector length.
- Each Triton program (vectorized execution unit) processes 1024 elements (indicated by the constant `%c1024_i32`), obtains the current block ID via `tt.get_program_id x`, and calculates its global offset.
