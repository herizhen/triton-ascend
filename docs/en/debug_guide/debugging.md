...
- Input File Cache: `ttir.mlir` file generated from the original Triton kernel

- Output File Cache: `ttadapter.mlir` file after Ascend adaptation conversion

- Compilation Artifact Cache: Final executable file generated by compilation
...
By setting `TRITON_DEBUG=1` to enable dump file output and `TRITON_DISABLE_CACHE=1` to disable caching and force recompilation, you can obtain `kernel.ttir.mlir` and `kernel.ttadapter.mlir`.
...
At the TTIR level, it is still based on Triton's native abstractions (e.g., `!tt.ptr<f32>`, `tt.load`/`tt.store`), not yet mapped to the specific memory model or execution units of the underlying hardware. It is a platform-independent high-level IR.
