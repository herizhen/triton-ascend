...
This document is the **Triton-Ascend Debugging Guide**, intended for engineers involved in the development of Triton and Ascend NPU adaptation. It systematically introduces common debugging methods and tools used during the Triton-Ascend compilation and execution process.

The content overview is as follows:

| Section | Main Content |
|------|--------|
| **1. Overview** | Explains the core debugging objective (focusing on `ttir.mlir` → `ttadapter.mlir` conversion) and provides a classification guide for common issues. |
...
Developers are advised to consult the relevant sections as needed based on specific issues to efficiently locate and resolve various anomalies in the Triton-Ascend integration.
...
| :--- | :--- | :--- |
| **Precision Issues** | NPU execution results differ from reference results (e.g., PyTorch or Triton CPU interpreter). | 4. Interpreter Mode <br> 5.1 Print Debugging Methods |
| **Compilation Errors (MLIRCompileError)** | Failure during the compilation/transformation phase, usually raising `MLIRCompileError` on the Python side. | 5.2 Compilation Error Debugging Methods |
...
| **Triton IR Adaptation Conversion** | `ttir.mlir` | `ttadapter.mlir` | Triton Backend Adapted for Ascend | **Key Debugging Stage**. Converts TTIR into Adapter IR for the Ascend NPU backend. |
...
**The debugging focus of this guide** is on the second stage: the conversion process from `ttir.mlir` to `ttadapter.mlir`. This stage is the core functionality of Triton-Ascend.

## 3 Triton-Ascend Temporary File Guide

During the Triton-Ascend compilation process, various temporary files are generated for caching and debugging. Understanding the location and purpose of these files is crucial for efficient debugging.
...
- Output File Cache: The `ttadapter.mlir` file after the Ascend adaptation conversion
...
Cache Verification: When suspecting cache-related issues, delete the relevant cache files and retest.
...
Understanding the generation timing and cleanup strategies of these temporary files helps manage the debugging environment better:
...
- TTIR Example
Viewing `kernel.ttir.mlir` shows:
...
- The function `@add_kernel` takes three pointer arguments (corresponding to the device memory addresses of input A, B, and output C) and an integer argument `n` representing the vector length.
...
- Vectorized loading is achieved via `tt.addptr` and `tt.load`, using mask `%6` (generated by `arith.cmpi slt`) to prevent out-of-bounds access.
...
Viewing `kernel.ttadapter.mlir` shows:
