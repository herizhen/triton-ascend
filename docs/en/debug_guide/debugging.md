...
|------|--------|
| **1. Overview** | Explains the core debugging objective (focusing on the `ttir.mlir` → `ttadapter.mlir` conversion) and provides a categorized guide for common issues. |
...
| :--- | :--- | :--- |
| **Precision Issues** | NPU execution results differ from reference results (e.g., PyTorch or Triton CPU Interpreter). | 4. Interpreter Mode <br> 5.1 Print Debugging Method |
| **Compilation Errors (MLIRCompileError)** | Failure during the compilation/transformation phase, typically throwing `MLIRCompileError` on the Python side. | 5.2 Compilation Error Debugging Method |
...
| **Triton IR Adaptation Conversion** | `ttir.mlir` | `ttadapter.mlir` | Ascend-adapted Triton Backend | **Key Debugging Stage**. Converts TTIR to Adapter IR targeting the Ascend NPU backend. |
...
**This guide's debugging focus** is on the second stage: the conversion process from `ttir.mlir` → `ttadapter.mlir`. This stage is the primary functionality of Triton-Ascend.

## 3 Triton-Ascend Temporary File Guide

During the Triton-Ascend compilation process, the system generates various temporary files for caching and debugging. Understanding the locations and purposes of these files is crucial for efficient debugging.
...
- Output File Cache: The `ttadapter.mlir` file after Ascend adaptation conversion

- Compilation Artifact Cache: The final compiled executable file
...
Cache Verification: When suspecting cache-related problems, delete the relevant cache files and retest.
...
Even if caching is enabled, as long as `TRITON_DEBUG=1` is set, the system will regenerate dump files on each run (overwriting files in the same-named directory). However, if the cache hits and compilation is skipped, the IR conversion may not be triggered, resulting in no new dumps. Therefore, it is recommended to also set the following during debugging:
...
Understanding the generation timing and cleanup strategies for these temporary files helps better manage the debugging environment:
...
By effectively utilizing these temporary files, developers can more efficiently locate and resolve issues encountered during Triton-Ascend compilation.
...
- Vectorized loads are performed via `tt.addptr` and `tt.load`, using the mask `%6` (generated by `arith.cmpi slt`) to prevent out-of-bounds access.
