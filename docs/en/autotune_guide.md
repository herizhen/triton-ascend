...
The order must be written as follows:
...
- Do not explicitly pass its value when launching the kernel;
...
While keeping this interface habit unchanged, Triton-Ascend mainly extends the following aspects.
...
Point 1 above explains that Triton-Ascend supports `configs=[]` for automatically generating candidate configurations. The following points elaborate on this capability.
...
The goal of this capability is to conveniently provide users with a Tiling configuration that performs reasonably well, while minimizing the user's usage threshold and optimization cost.
