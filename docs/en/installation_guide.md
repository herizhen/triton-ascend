...
- Step 1: Install LLVM using clang. Please install clang and lld on the environment and specify the version (recommended version clang>=15, lld>=15).
  If not installed, please install clang, lld, and ccache using the following commands:
...
- Step 3: Execute the following commands to build and install LLVM:
  ...
  ```bash
  cd {PATH_TO}/llvm_project # Path to the directory where LLVM code is pulled; adjust as needed
  mkdir build
  cd build
  cmake ../llvm \
    -G Ninja \
...
- Step 1: Ensure that the target path for LLVM installation, ${LLVM_INSTALL_PREFIX}, from the [Build Based on LLVM] section is set.
