...
- **Quick Setup with Docker Image**: Use the ready-to-use image released by Triton-Ascend to quickly build a development environment. Follow the instructions in [OVERVIEW.md](../../docker/OVERVIEW.md) directly.
...
When installing Triton-Ascend, the community Triton is installed first, and then Triton-Ascend overwrites the directory with the same name, thus preventing subsequent installation of other packages that depend on Triton from reinstalling Triton and overwriting Triton-Ascend.
...
Building Triton-Ascend strongly depends on zlib1g-dev. If you are using a yum source, please refer to the following command to install:
...
# Optional: If you have a locally compiled LLVM, you can directly specify the local LLVM path without triggering the download of the LLVM precompiled package. If not, ignore this and proceed directly with the installation command below.
...
##### Code Preparation: `git checkout` to check out the specified version of LLVM
...
- Step 3: Execute the following commands to build and install LLVM:
  ...
  ```bash
  cd {PATH_TO}/llvm_project # The path is the path where the user pulled the LLVM code, needs to be adjusted according to the actual situation
  mkdir build
  cd build
  cmake ../llvm \
    -G Ninja \
