...
- **Quick Setup with Docker Image**: Use the ready-to-use image published by Triton-Ascend to quickly build a development environment. Follow the instructions in [OVERVIEW.md](../../docker/OVERVIEW.md);
...
Note: For more images, see [OVERVIEW.md](../../docker/OVERVIEW.md).
...
```bash
# Assuming your NPU device model is A3, installed on /dev/davinci1, and your NPU driver is installed at /usr/local/Ascend:
...
The heterogeneous computing architecture CANN (Compute Architecture for Neural Networks) is Ascend's heterogeneous computing architecture for AI scenarios. It supports multiple AI frameworks upward, including MindSpore, PyTorch, TensorFlow, etc., and serves AI processors and programming downward, playing a key role in improving the computing efficiency of Ascend AI processors.
...
The current compatible torch_npu version is 2.7.1.post4.

```bash
pip install torch_npu==2.7.1.post4
```

Note: If you encounter the error `ERROR: No matching distribution found for torch==2.7.1+cpu`, try manually installing torch before installing torch_npu.
...
When installing Triton-Ascend, the community Triton is installed first, and then Triton-Ascend overwrites the same-named directory, thus preventing other packages that depend on Triton from reinstalling Triton and overwriting Triton-Ascend.
...
We recommend using the <a href="#auto-code-base">Quick Installation</a> method to install Triton-Ascend from source. If you have special requirements, such as the target machine not having network access, you can proceed with <a href="#hand-code-base">Manual Installation</a>.
...
Triton-Ascend build strongly depends on zlib1g-dev. If you use a yum source, please refer to the following command to install:
...
# Optional: If you have a locally compiled LLVM, you can directly specify the local LLVM path to avoid downloading the LLVM precompiled package. If not, ignore this and proceed with the installation command below.
...
Triton uses LLVM 22 to generate code for GPU and CPU. Similarly, Ascend's Bisheng Compiler also relies on LLVM to generate NPU code, so you need to compile the LLVM source code to use it. Pay attention to the specific LLVM version required. LLVM build supports two build methods. **Choose one of the following two methods**; no need to execute both.
...
- Step 1: Install LLVM using clang. Ensure clang and lld are installed on the system, and specify the version (recommended clang>=15, lld>=15).
...
- Step 1: Ensure the LLVM installation target path ${LLVM_INSTALL_PREFIX} from the [Build Based on LLVM] section is set.
- Step 2: Ensure clang>=15, lld>=15, and ccache are installed.
...
Note 1: The recommended GCC version is in the "System Recommendations" section above. If GCC < 9.4.0, you might encounter the error "ld.lld: error: unable to find library -lstdc++fs", indicating the linker cannot find the stdc++fs library.
This library supports filesystem features for versions before GCC 9. In this case, manually uncomment the relevant code snippet in the CMake file:
...
We provide a Dockerfile to help you install a Docker environment image. The build process uses the `quay.io/ascend/cann` pre-built image as the base image, skipping the CANN installation step and significantly speeding up the build.
