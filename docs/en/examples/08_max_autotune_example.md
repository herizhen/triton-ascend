...
`max_autotune` is an enhanced auto-tuning decorator provided by Triton-Ascend, designed to simplify the code writing for multi-parameter tuning. Unlike the community version `autotune`, which requires users to manually enumerate all `triton.Config` entries, `max_autotune` allows users to provide only a few basic configurations (e.g., block sizes) and automatically includes related compiler options (such as `num_stages`, `enable_hivm_auto_cv_balance`, etc.) into the search space for optimal combinations. Users can also explicitly control the search range through parameter lists.
...
def test_max_autotune():
    ...
    # Basic configurations: only block sizes are needed; other tuning parameters are auto-generated by the decorator
    base_configs = [
