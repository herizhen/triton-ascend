...
```Python
# The following explains the key parameter usage points for advanced autotune compared to the community version
#
# configs:
...
# - Advanced autotune automatically generates candidate tiling configurations based on the kernel, then compiles and benchmarks each configuration to select the optimal one.
...
#       2. Users can pass parameters via hints to generate tiling, involving split_params, tiling_params, low_dim_axes, and reduction_axes. All four parameters must be provided together.

# split_params (Dict[str, str]): A dictionary of axis name: argument name, where the argument is a tunable parameter for the split axis, e.g., 'XBLOCK'
#     The axis name must be in the set of axis names for the parameter keys. Do not prefix the axis name with 'r'.
...
#     The split axis can usually be determined by the `tl.program_id()` kernel distribution statement.
# tiling_params (Dict[str, str]): A dictionary of axis name: argument name, where the argument is a tunable parameter for the tiling axis, e.g., 'XBLOCK_SUB'
#     The axis name must be in the set of axis names for the parameter keys. Do not prefix the axis name with 'r'.
#     This parameter can be empty. If both split_params and tiling_params are empty, automatic optimization will not be performed.
...
# reduction_axes (List[str]): A list of axis names for all reduction axes. The axis name must be in the set of axis names for the parameter keys. Prefix the axis name with 'r'.
# auto_gen_config (bool): Default is False. Involves the following scenario combinations:
...
#     2. If the user defines Config and auto_gen_config=False, the framework does not automatically generate Config, using only the user-defined Config;
...
# - Pass a list of runtime parameter names; a change in any parameter value in the list triggers the regeneration and evaluation of candidate configurations.
...
#       2. If hints does not pass information for split_params, tiling_params, low_dim_axes, and reduction_axes, the key type must be list[str]. The axis information will be distributed according to the parameter order, as shown in Example 2:
...
def add_kernel(
    ...
    # The offset of the data this kernel will process relative to the starting address in memory.
...
1. Triton-Ascend defaults to using a benchmark method to measure on-chip computation time. When the environment variable `export TRITON_BENCH_METHOD="npu"` is set, it uses `torch_npu.profiler.profile` to obtain the on-chip computation time for each kernel configuration. For Triton kernels with fast computation, such as small-shape operators, this can provide more accurate computation time compared to the default method, but it significantly increases the overall autotune time. Use with caution.
...
Before automatic parameter parsing, the system first obtains the parameters **not passed** during the `kernel` function call. **These unpassed parameters are considered candidates for split axis and tiling axis parameters**.
...
Finally, the tiling axis corresponding to the current parameter is confirmed through mask comparison and the `key` passed into `autotune`.
...
Low-dimensional axis parameter parsing is determined based on `tl.arange()` tiling statements. The system identifies potential low-dimensional axis parameters by analyzing the usage of `tl.arange()` in the program and the variables it computes. It extracts `tl.arange()` itself and the variables it participates in calculating, filters them by checking whether slicing operations are performed for dimension augmentation, and by judging the augmented dimension.

Finally, the low-dimensional axis of the current kernel is confirmed through mask comparison and the `key` passed into `autotune`.

Note: 1. The low-dimensional axis must be computed via `tl.arange()` and sliced. It will only be recognized if it undergoes dimension expansion in a non-low