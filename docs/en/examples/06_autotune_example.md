...
#
# hints(Dict[str, str]):
# Note: 1. hints is optional. When not provided by the user, the framework automatically parses relevant parameters like split_params, tiling_params, etc.
...
# - Pass a list of runtime parameter names; a change in any parameter value in the list triggers regeneration and evaluation of candidate configurations.
...
#       2. If hints does not pass information for split_params, tiling_params, low_dim_axes, and reduction_axes, the key type must be list[str]. Axis information will be assigned according to the parameter order, as shown in Example 2:
...
1. Triton-Ascend defaults to using the benchmark method to obtain on-chip computation time. When the environment variable `export TRITON_BENCH_METHOD="npu"` is set, it uses `torch_npu.profiler.profile` to obtain the on-chip computation time for each kernel configuration. For some Triton kernels with fast computation, such as small shape operators, this can obtain more accurate computation time compared to the default method, but it will significantly increase the overall autotune time. Enable with caution.
...
Before performing automatic parameter parsing, it first obtains the parameters not passed during the `kernel` function call, **treating the unpassed parameters as candidates for split axis and tiling axis parameters**.
