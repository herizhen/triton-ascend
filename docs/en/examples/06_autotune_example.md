...
def test_triton_autotune():
    ...
    def triton_calc_kernel(
        ...
        msk = idx < numel                 # Mask to prevent out-of-bounds access
...
```Python
# The following explains the key parameter usage points for advanced autotune vs. community version
#
# configs:
# - Community autotune (default) requires explicitly passing a set of triton.Configs. The framework compiles and benchmarks each configuration to select the optimal one.
# - Advanced autotune framework automatically generates candidate tiling configurations based on the kernel, then compiles and benchmarks each configuration to select the optimal one.
...
#
# hints(Dict[str, str]):
# Note: 1. hints is optional. If not provided, the framework will automatically parse relevant parameters like split_params, tiling_params, etc.
#       2. Users can pass parameters via hints to generate tiling, involving split_params, tiling_params, low_dim_axes, reduction_axes. All four parameters must be provided together.

# split_params (Dict[str, str]): A dictionary of axis name: argument name, where argument is the tunable parameter for the split axis, e.g., 'XBLOCK'
#     The axis name must be in the set of axis names for the parameter key. Do not prefix axis names with r.
#     This parameter can be empty. If both split_params and tiling_params are empty, automatic optimization will not occur.
#     Split axes can usually be determined by the `tl.program_id()` kernel dispatch statement.
# tiling_params (Dict[str, str]): A dictionary of axis name: argument name, where argument is the tunable parameter for the tiling axis, e.g., 'XBLOCK_SUB'
#     The axis name must be in the set of axis names for the parameter key. Do not prefix axis names with r.
#     This parameter can be empty. If both split_params and tiling_params are empty, automatic optimization will not occur.
#     Tiling axes can usually be determined by the `tl.arange()` tiling expression.
# low_dim_axes (List[str]): A list of axis names for all low-dimensional axes. Axis names must be in the set of axis names for the parameter key.
# reduction_axes (List[str]): A list of axis names for all reduction axes. Axis names must be in the set of axis names for the parameter key. Prefix axis names with r.
...
# - Pass a list of runtime parameter names; a change in any parameter value in the list will trigger regeneration and evaluation of candidate configurations.
# Note: 1. If hints pass information for split_params, tiling_params, low_dim_axes, reduction_axes, the key type must be Dict[str,str], as shown in Example 1:
#       2. If hints do not pass information for split_params, tiling_params, low_dim_axes, reduction_axes, the key type must be list[str], and axis information will be allocated according to parameter order, as shown in Example 2:
...
def add_kernel(
    ...
    pid = tl.program_id(axis=0)  # We use a 1D grid, so axis is 0.
    ...
    # Load x and y, masking out extra elements in case the input vector length is not a multiple of block size.
...
Note: 1. A low-dimensional axis **must** be computed via `tl.arange()` and undergo slicing. It must be dimensionally expanded in a non-lowest dimension