...
| **Debug & Logging** | TRITON_INTERPRET | 0 or unset | Uses the Triton interpreter instead of GPU execution, allowing Python breakpoints to be inserted in kernel code. | 0: Breakpoints not supported<br>1: Breakpoints supported | |
...
| **Debug & Logging** | TRITON_PRINT_AUTOTUNING | 0 or unset | Outputs the best configuration and total time for each kernel after autotuning completes. | 0: Do not output<br>1: Output | |
...
| **Compilation Control** | TRITON_ALWAYS_COMPILE | 0 or unset | Controls whether Triton forces recompilation of kernels on every run instead of using cached versions. By default, Triton caches compiled kernels (based on parameters and configuration) to improve performance. When set to 1, Triton ignores the cache and recompiles kernels every time, which is useful for debugging or testing new compiler features. | 0: Disable<br>1: Recompile all kernels on every run | |
...
| **Compilation Control** | TRITON_KERNEL_OVERRIDE | 0 or unset | Enables or disables the Triton kernel override feature, allowing user-specified external files (IR/PTX, etc.) to override the default generated kernel code at the beginning of each compilation stage. | 0: Disable<br>1: Enable | |
...
| **Compilation Control** | TRITON_ASCEND_COMPILE_SPEED_OPT | 0 or unset | Controls whether the JIT compiler skips subsequent compilation stages after a kernel compilation failure. Set to `1` to skip (default `0` continues attempting). | 0: Continue attempting<br>1: Skip | |
...
| **Runtime & Scheduling** | ENABLE_PRINT_UB_BITS | 0 or unset | When enabled, allows retrieving the current UB usage, for use by inductor. | 0: Disable<br>1: Enable | |
...
The compilation option configuration is detailed in the table below:
...
| **Synchronization** | `unit_flag` | `None`, `True`, `False` | Cube outbound related synchronization optimization item. | `triton.Config` or Autotune parameter |
...
| **Multi-buffer Scope** | `limit_auto_multi_buffer_only_for_local_buffer` | `None`, `True`, `False` | Restricts automatic multi-buffer to only apply to local buffers. | `triton.Config` or Autotune parameter |
| **Multi-buffer Scope** | `limit_auto_multi_buffer_of_local_buffer` | `None`, `"no-limit"`, `"no-l0c"` | Configures the scope of automatic multi-buffer for local buffers. | `triton.Config` or Autotune parameter |
