...
We provide comprehensive online documentation and resources covering environment setup, operator development, tuning practices, and frequently asked questions to help users get started quickly and use the tool effectively. For details, please refer to the [Online Documentation](https://triton-ascend.readthedocs.io/zh-cn/latest/index.html)
...
Users can follow the steps in the [Installation Guide](installation_guide.md) environment preparation section to set up the Triton-Ascend environment.
...
- Note 1: This solution is used to mitigate installation override issues and cannot completely eliminate the conflict caused by the shared `triton` package directory between community Triton and Triton-Ascend; if subsequent installation processes explicitly reinstall or upgrade community Triton, it may still affect the installed Triton-Ascend. In this case, please uninstall both community Triton and Triton-Ascend first, and then reinstall Triton-Ascend.
...
For different chip types, the corresponding machines can be referenced as follows:
...
Observing output similar to the following indicates that the environment is configured correctly.
