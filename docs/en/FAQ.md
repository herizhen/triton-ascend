...
When installing Triton-Ascend, the community Triton is installed first, then Triton-Ascend overwrites the directory with the same name, thus preventing subsequent installations of other packages that depend on Triton from overwriting Triton-Ascend.
...
A: The CI checks for a PR include: code security and specification checks, open-source fragment checks, malicious code checks, compilation and build, and developer tests.
