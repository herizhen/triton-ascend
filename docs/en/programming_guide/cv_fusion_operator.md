...
Simple CV fusion can start from `third_party/ascend/tutorials/03-matrix-multiplication.py` with matmul + activation, or refer to the [fused attention example](../examples/04_fused_attention_example.md). The minimal path is as follows:
...
For attention-type CV fusion, it is recommended to first get non-causal, short-sequence, small-head_dim cases working, then gradually add:
