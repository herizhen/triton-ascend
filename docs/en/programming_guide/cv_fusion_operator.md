...
When developing simple CV fusion, maintain clear boundaries: Cube is responsible for producing large 2D accumulators, while Vector handles element-wise or small-scale reductions within the same tile. If the Vector part requires shared state across multiple Cube tiles, synchronization, workspace, or kernel splitting must be introduced.
