...
2. Use `mask` to protect the tail block, preventing out-of-bounds load/store.
...
- **BLOCK_SIZE**: BLOCK_SIZE should be as large as possible within the UB capacity. If a UB overflow occurs, first reduce the number of elements processed at once, then consider splitting into sub-blocks.
- **Number of Cores**: The number of physical Vector Cores on an NPU is typically a few dozen. When migrating GPU code with small tiles and large grids to NPU, significant overhead can arise from multiple rounds of dispatch.
...
5. **Reserve a unified mask for tail blocks**: In complex gather/scatter, there are index masks, column masks, and expert/bin boundaries simultaneously. It is recommended to name them separately and only combine them at load/store points.
...
- Whether the trailing axis satisfies 32B alignment; if not, whether transposition or borrowing axis transposition can be used to avoid automatic padding.
