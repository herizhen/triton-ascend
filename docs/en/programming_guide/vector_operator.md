...
- **Number of Cores**: The number of physical Vector Cores on an NPU is typically a few dozen. When migrating GPU code with small tiles and large grids to NPU, the overhead from multiple dispatch rounds can be significant.
...
5. **Reserve a unified mask for tail blocks**: In complex gather/scatter, there are simultaneous index masks, column masks, and expert/bin boundaries. It is recommended to name them separately and only combine them at load/store points.
...
- Whether `BLOCK_X` and `SUB_BLOCK_SIZE` cause UB overflow or excessively small transfer granularity.