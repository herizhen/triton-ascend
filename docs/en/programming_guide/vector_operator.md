...
-   **Number of Cores**: The number of physical Vector Cores on an NPU is typically several dozen. When migrating GPU code with small tiles and large grids to NPU, significant overhead can arise from multiple dispatch rounds.
...
The organization method for this type of operator is typically:
...
3.  **Merge small-grained discrete tasks using `SUB_BLOCK_SIZE`**: Load a group of indices at once, organize them into contiguous temporary blocks in UB, reducing GM scalar memory access and multiple stores.
4.  **Manage local data within UB using extended semantics**: Use `tl.insert_slice` to merge multiple rows of data, and `tl.extract_slice` to extract sub-blocks before scattering writes.
...
A typical UB budget estimation approach is as follows:
...
-   Whether the trailing axis satisfies 32B alignment; if not, whether transposition or axis borrowing can be used to avoid automatic padding.
