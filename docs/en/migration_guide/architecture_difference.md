...
Developers can manually select the optimal tiling configuration based on the actual scenario to maximize on-chip memory utilization and avoid performance bottlenecks caused by frequent global memory access.
...
Note: The Atlas 800T/I A2 product has an on-chip memory capacity of 192KB, so this limit must be considered when designing the tiling strategy to ensure that the data volume per computation round does not exceed the on-chip memory capacity.
...
# Further subdivide the data block within a single core, processing XBLOCK_SUB size data each iteration, implementing intra-core tiling.
for xoffset_sub in range(0, XBLOCK, XBLOCK_SUB):
...
| unit_flag | An optimization item for cube data movement | Default None; true, false. Configurable in autotune. |
| limit_auto_multi_buffer_only_for_local_buffer | An optimization item for CV operators, an optimization item for cube data movement | Default None; true, false. Configurable in autotune. |
...
| set_workspace_multibuffer | Takes effect only when limit_auto_multi_buffer_only_for_local_buffer=false | Default None; e.g., [2,4], configurable in autotune. |
| enable_hivm_auto_cv_balance | set_workspace_multibuffer takes effect only when limit_auto_multi_buffer_only_for_local_buffer=false | Default None; true, false. Configurable in autotune. |
