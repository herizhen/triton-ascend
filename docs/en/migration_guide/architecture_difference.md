...
- Core Comparison
    ...
    |Core Count / Dimension Limits| No hard limits on grid dimensions/size| Grid size ≤ total AI Cores, 2D must match topology|
...
NPU: Vector cores and Cube cores are multiple physical cores, with different core counts across hardware generations. Each core executes one Block only once and supports repeated scheduling of that Block.
...
Note: The Atlas 800T/I A2 product has an on-chip memory capacity of 192KB. Therefore, tiling strategies must consider this limit to ensure that the data volume per compute round does not exceed the on-chip memory capacity.
...
The following is a simple Triton kernel example demonstrating how to define and call a basic Triton kernel function. This example implements a simple mathematical operation (GELU activation function).
...
# Set a mask to prevent out-of-bounds access, ensuring only valid data is processed.
xmask = x_index < xnumel
