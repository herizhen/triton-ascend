...
By tuning the core count, you can fully schedule and utilize all computing resources, maximizing parallelism and throughput. Note that in the current version, the core count must be ≤ 65535.
...
xblock_sub: Intra-core tiling granularity (fine-grained intra-core division)
```
...
Note: The Atlas 800T/I A2 product has an on-chip memory capacity of 192KB. Therefore, this limitation must be considered when designing the tiling strategy to ensure that the data volume per computation round does not exceed the on-chip memory capacity.
...
Input tensor x0, compute the GELU operator via Torch, and return the result.
...
Taking enabling the `multibuffer` option as an example, pass `'multibuffer': True` in `triton.Config` during the autotune configuration phase. See [autotune example](../examples/06_autotune_example.md) for details:
...
| tile_mix_vector_loop | An optimization item for CV operators, specifies how many parts the current vector can be divided into | Default None; e.g., [2,4,8], configurable in autotune. |
| tile_mix_cube_loop | An optimization item for CV operators, specifies how many parts the current cube can be divided into | Default None; e.g., [2,4,8], configurable in autotune. |
