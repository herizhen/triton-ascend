...
- Core Comparison
    ...
    |Core Count / Dimension Limit| No hard limit on grid dimensions/size| grid size ≤ total AI Cores, 2D must match topology|
...
NPU: Vector cores and Cube cores belong to multiple physical cores, with different numbers of cores across hardware generations. Each core executes only one Block and supports repeated scheduling of that Block.
