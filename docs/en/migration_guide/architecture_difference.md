...
By tuning the core count, you can fully schedule and utilize all compute resources, thereby maximizing parallelism and throughput. Note: In the current version, the core count must be ≤ 65535.
...
Input tensor x0, compute the GELU operator via torch computation, return the result value.
...
Compiler options for AscendNPU IR optimization have been adapted to Ascend's hardware and software characteristics, as shown in the table below.
...
| auto_blockify_size                            | TRITON_ALL_BLOCKS_PARALLEL optimization item, used to specify the size of the extended leftmost first dimension | Default 1; e.g., [2,4,8], configurable in autotune                       |

- Note: The optimization compiler options are in the ascend/backend/compiler.py code.
