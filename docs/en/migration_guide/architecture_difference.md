...
When calling Triton kernel functions, control the number of cores used by setting launch parameters. Take the GELU operator as an example:

```Python
triton_gelu[n, 1, 1](...)  # First parameter indicates number of cores used, n means using n cores
```

By tuning the core count, you can fully schedule and utilize all computing resources, maximizing parallelism and throughput. Note: In the current version, the core count must be ≤ 65535.
...
xblock_sub: Intra-core tiling granularity (fine-grained division within a core)
```
...
Note: The Atlas 800T/I A2 product has an on-chip memory capacity of 192KB. Therefore, tiling strategy design must consider this limitation to ensure the data volume per computation round does not exceed the on-chip memory capacity.
...
1. Memory Limitation: In the above implementation, all input data is loaded into memory at once for computation. If the input tensor is too large, it may exceed the on-chip memory capacity of a single kernel, causing memory overflow errors. Therefore, this simple approach is more suitable for small-scale tensor computation or for understanding the basic syntax and calling methods of Triton kernels.

2. Applicable Scenarios: Although this method helps quickly understand and get started with Triton programming, for large-scale datasets or high-performance applications, it is recommended to adopt more complex data tiling strategies (e.g., Tiling) to fully utilize hardware resources and avoid memory overflow issues. This approach allows developers to quickly get started with Triton programming while understanding how to define, call, and optimize Triton kernel functions.
...
Tailored to Ascend's hardware and software characteristics, the following compilation options for AscendNPU IR optimization are provided.
...
| unit_flag | An optimization for Cube data output | Default None; true, false. Configurable in autotune |
| limit_auto_multi_buffer_only_for_local_buffer | An optimization for CV operators, an optimization for Cube data output | Default None; true, false. Configurable in autotune |
...
| tile_mix_vector_loop | An optimization for CV operators, specifies how many parts the current vector can be divided into | Default None; e.g., [2,4,8], configurable in autotune |
| tile_mix_cube_loop | An optimization for CV operators, specifies how many parts the current cube can be divided into | Default None; e.g., [2,4,8], configurable in autotune |
...
- Note: Optimization compilation options are located in the `ascend/backend/compiler.py` code.
