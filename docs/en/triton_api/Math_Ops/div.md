...
The underlying implementation is the same as the `fdiv` operator, except that `fdiv` explicitly requires the input to be of `float` type, while '/' has no such restriction. It converts non-floating-point types to floating-point before computation.
...
The following example demonstrates performing division on input tensors `in_ptr0` and `in_ptr1`:
