...
Description: Performs matrix multiplication on two tensors. The tensors must be 2D or 3D with consistent dimensions. For 3D blocks, `tl.dot` performs batched matrix multiplication, where the first dimension of each block represents the batch dimension.
Prototype:
...
| `max_num_imprecise_acc`     | `int`    | Number of imprecise accumulations (currently not supported on Ascend for low-precision accumulation). |
