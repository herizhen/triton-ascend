...
Description: Performs matrix multiplication on two tensors. The tensors must be two-dimensional or three-dimensional with consistent dimensions. For three-dimensional blocks, `tl.dot` performs batch matrix multiplication, where the first dimension of each block represents the batch dimension.
Prototype:
...
| `acc`          | `int32 float32`     | Accumulator tensor. If not None, the result is added to this tensor. Supported `acc_dtype`: `float16`, `float32`, `int32`. |
| `input_precision` | -                 | Available options for NVIDIA. Determines whether Tensor Cores acceleration is enabled by selecting a precision mode. |
