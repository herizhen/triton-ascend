...
| `scope`        | `str`, optional     | Thread scope for observing atomic operation synchronization effects<br>Acceptable values are "gpu" (default), "cta" (cooperative thread array, thread block), or "sys" (representing "SYSTEM")<br>We only support "gpu" |
...
The following example implements atomic XOR computation:
