...
Description: Atomic exchange operation that performs an atomic swap at a specified memory location.
Prototype:
...
| `scope` | `str`, optional | The thread scope for observing the synchronization effects of the atomic operation<br>Acceptable values are "gpu" (default), "cta" (cooperative thread array, thread block), or "sys" (representing "SYSTEM") <br>We only support "gpu"                                            |
