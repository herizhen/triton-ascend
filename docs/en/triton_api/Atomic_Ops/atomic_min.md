...
Description: Atomic minimum operation, performs an atomic min operation at a specified memory location.
Prototype:
...
Can be called as a member function of a tensor, e.g., `x.atomic_min(...)`, which is equivalent to `atomic_min(x, ...)`.
...
| -------------- | ---- | ----------- |
| `pointer` | `triton.PointerDType` | The memory location to operate on. The result of computing min(*pointer, val) is written back to this memory location. |
| `val` | `pointer.dtype.element_ty` | The value for the atomic minimum operation (right operand). |
...
The following example implements an atomic minimum calculation:
