...
| `strides`    | `List[tensor]`      | Stride list for each dimension of the tensor, with constraints: - The preceding dimensions must be multiples of 16 bytes - The last dimension must be contiguous |
...
Return value:
`tensor_descriptor`: Tensor descriptor object (cannot be directly used for arithmetic operations; must be used with `load` / `store`)
...
| Triton version compatibility  | Triton 3.2.0 has compatibility issues with some functions (e.g., `cast`). It is recommended to upgrade the Triton version to 3.4.0 to fix binding restrictions. | Upgrade to Triton 3.4.0 |
