...
| `boundary_check` | `tuple(int)` | Optional parameter, can only be passed when `pointer` originates from `tl.make_block_ptr`<br>An integer tuple indicating the dimensions that require boundary checking                                         |
...
1. If `pointer` is a single pointer:
   - Then `value` and `mask` must be scalars
...
| -------------------------------------- | ------------------------------------------------------------ | ------------------------------ |
| Generalization issue with discrete mask | Currently, the handling of discrete masks in store involves decomposing the store into atomic {load, select, store}, which has certain generalization issues in corner cases | Expose issues through extensive generalization testing, resolve iteratively |
| Generalization issue when used with branch and loop statements | The computation process of `pointer` and `mask` in `tl.load`, if involving complex loops and branch statements, may cause compilation issues | Expose issues through extensive generalization testing, resolve iteratively |
