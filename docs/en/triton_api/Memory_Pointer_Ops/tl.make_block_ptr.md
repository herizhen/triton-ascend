...
| `order`      | `tuple(constexpr)`  | Order of dimensions for loading/storing the block from/to global memory |
...
| -------------------------------------------------- | ------------------------------------------------------------------------------------------- | ----------------------------------------- |
| Generalization issue when used with branches/loops | Currently, `tl.make_block_ptr` may cause compilation issues when used with complex loops and branches | Expose issues through extensive generalization testing, resolve iteratively |
