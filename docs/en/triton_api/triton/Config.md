...
- **kwargs** – A dictionary of meta-parameters to be passed to the kernel as keyword arguments.

- **num_warps** – The number of warps used by the kernel when compiled for the GPU. For example, if num_warps=8, each kernel instance will be automatically parallelized to execute collaboratively using 8 * 32 = 256 threads.
