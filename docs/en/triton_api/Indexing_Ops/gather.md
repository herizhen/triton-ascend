...
Description: Performs a gather operation on the `src` tensor along the `axis` dimension according to the `index`. The meaning of the gather operation is illustrated below:
![image](./gather.png)
Prototype:
