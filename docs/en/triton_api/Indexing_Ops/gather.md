...
Description: Performs a gather operation on the `src` tensor along the `axis` dimension using the `index` tensor. The meaning of the gather operation is illustrated below:
![image](./gather.png)
Prototype:
