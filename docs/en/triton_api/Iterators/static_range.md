...
|-----------|------|---------|-------------|
| `arg1` | `constexpr` | Required | Start value (when used as single parameter, it serves as the end value, starting from 0) |
...
`static_range` trades code size for runtime performance, suitable for known and small loop iteration counts.