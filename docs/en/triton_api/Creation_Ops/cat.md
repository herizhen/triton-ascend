...
| `_semantic` | `Optional[str]` | Reserved parameter, external calls not supported yet |
...
Conclusion: There is no difference in Shape between GPU and Ascend platforms. `cat` only supports 1D shape concatenation.
