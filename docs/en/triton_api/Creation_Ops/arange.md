...
Conclusion: The parameters `start` and `end` of `arange` must be constants, hence they have no type. The supported types correspond to value ranges up to int32, and hardware instructions also only support up to int32.
...
3. The inputs to `arange` must be constant values, supporting uint and int type values less than 1048576 (the maximum value TRITON_MAX_TENSOR_NUMEL). int64 is not supported.
