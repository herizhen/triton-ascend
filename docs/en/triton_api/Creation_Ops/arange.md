...
Conclusion: The `start` and `end` parameters of `arange` must be constants, thus they have no type. The supported value range is up to `int32`, and hardware instructions also only support up to `int32`.
