...
| -------------- | ----------- | ------------------------------------------------ |
| `start`        | `scalar`    | Starting value for the contiguous integer sequence, must be a compile-time constant (`tl.constexpr`) |
| `end`          | `scalar`    | Ending value for the contiguous integer sequence  |
...
Conclusion: The `start` and `end` parameters of `arange` must be constants, hence they have no type. The supported value range extends up to `int32`, and hardware instructions also only support up to `int32`.
...
> Relative community feature missing and not implementable
...
3. The inputs to `arange` must be constant values, supporting `uint` and `int` type values less than 1048576 (maximum `TRITON_MAX_TENSOR_NUMEL`). `int64` is not supported.
4. The `start` and `end` values of `arange` must be greater than or equal to 0.
