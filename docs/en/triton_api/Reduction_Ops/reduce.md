...
Description: `triton.language.reduce` applies `combine_fn` along the specified `axis` to reduce the input tensor, returning the reduced result tensor.
...
Return value:
`tensor`: The result tensor after reduction along the specified `axis`.
