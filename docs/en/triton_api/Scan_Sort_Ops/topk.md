...
| `dim` | `constexpr int` or `None` | Dimension along which to find the top k elements; must be determined at compile time; if `None`, the last dimension is used; currently only the last dimension is supported |

Return value:
`out`: The shape of the output tensor is the same as the input tensor, except that the specified dimension length becomes `k`
