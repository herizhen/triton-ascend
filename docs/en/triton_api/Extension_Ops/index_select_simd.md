...
- Efficient zero-copy implementation, directly transferring from GM to UB
- Preserves element type and encoding unchanged

**Typical Use Cases:**

- Embedding lookup: Reading word vectors in batches based on token IDs from a large vocabulary
...
- `index` must be a 1D tensor
- The size of each dimension of the source tensor must be within actual hardware memory limits
- The size of `read_shape` for non-dim dimensions must consider UB space limitations
