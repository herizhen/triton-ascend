...
Inter-core synchronization control interface for separation mode.

This interface works in conjunction with the sync_block_wait interface. When used, a synchronization flag ID (flagId) must be passed, where each ID corresponds to a counter with an initial value of 0. After executing CrossCoreSetFlag, the counter corresponding to the ID increments by 1. When executing CrossCoreWaitFlag, if the corresponding counter value is 0, execution blocks; if the corresponding counter is greater than 0, the counter decrements by 1, and subsequent instructions begin execution.
...
<table>
  ...
  <tr>
    <td>PIPE_S</td>
    <td>Scalar pipeline, used when calling Tensor GetValue function</td>
  </tr>
  <tr>
    <td>PIPE_V</td>
