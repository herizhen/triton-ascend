...
This interface is used in conjunction with the sync_block_set interface. When used, the synchronization flag ID (flagId) must be passed. Each ID corresponds to a counter with an initial value of 0. After executing CrossCoreSetFlag, the counter corresponding to the ID increases by 1; when executing CrossCoreWaitFlag, if the corresponding counter value is 0, it blocks and does not execute; if the corresponding counter is greater than 0, the counter decreases by 1, and subsequent instructions begin execution.
