...
The equivalent number of configurations after expansion for the above setup is:
...
> Note: `kernel_type="mix"` supports many parameters. Parameters not explicitly provided will use default values and participate in the expansion. To prevent a specific parameter from being expanded, fix its value in the base `Config`'s `kwargs`.
...
To prevent a specific tuning parameter from being expanded, fix it directly in the base configuration:
...
Too many configurations will increase the initial tuning time. It is recommended to control the parameter space reasonably based on actual needs.
...
1. The tiling parameter space is known, and the goal is to reduce the workload of manually enumerating configurations;
2. Joint tuning of multiple Ascend compilation parameters (such as `num_stages`, `enable_hivm_auto_cv_balance`, etc.) is required;
3. Batch generation of candidate configurations via Cartesian product is desired.

The core value of `max_autotune` lies in: using a small number of base configurations plus a description of the tuning parameter space to automatically expand into a complete set of candidate configurations, balancing flexibility and convenience.