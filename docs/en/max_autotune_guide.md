...
| `unit_flag` | ✅ | ✅ | ❌ | `[False]` | Boolean list | Cube transfer-out related synchronization optimization |
...
1. **tuning_params parameter** (highest priority): The list of candidate values passed via `**tuning_params`;
...
@max_autotune(
    ...
    kernel_type="vector",
    num_stages=[1, 2],
    # enable_ubuf_saving is already fixed to False in the base configuration, will not use default [True]
)
@triton.jit
def kernel(...):
    ...
...
| Applicable scenario | Know the tiling parameter space, need to tune compilation parameters | Want tiling parameters to also be automatically generated |
...
1. You know the tiling parameter space and want to reduce the effort of manually enumerating configurations;
