...
- State in one sentence what the document aims to do and the goal to be achieved.
- Specify the vLLM-Ascend version used in the document and the model version support status.
...
Introduce the features supported by the model, including supported hardware, quantization methods, data parallelism, long sequence characteristics, etc.
...
- Version number writing specification: Prioritize using placeholders (values configured uniformly); if using a fixed value that differs from the document's verification version, add a comment "Please replace according to the actual version".
...
Common issue prompt: If encountering xxx issues, please refer to the [Public FAQ](https://docs.vllm.ai/projects/ascend/en/latest/faqs.html) for troubleshooting.
...
| --------- | --------- | --------- |
| Rope Optimization | The position encoding's cos_sin_cache and index operations are only performed in the first layer; subsequent layers directly reuse them | Reduces repeated calculations during the decode phase, accelerating inference |
...
- Add a note at the beginning of the section: For common environment, installation, and general parameter issues, please refer to the [Public FAQ](https://docs.vllm.ai/projects/ascend/en/latest/faqs.html); this section only covers specific difficult issues related to this model.
- For **specific difficult issues related to this model**, provide the following elements: Problem description, cause analysis, and solution measures.