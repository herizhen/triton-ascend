...
- State in one sentence what the document is intended to do and the goal to be achieved.
...
- Describe the principles and applicable scenarios of the PD separation architecture.
...
- Guide users on how to test whether the model's basic functions are normal through simple interface tests after the service is started.
...
| Matmul-ReduceScatter Fusion | Large-scale distributed environments | Automatically enabled after enabling FlashComm_v1 | Merges matrix multiplication with Reduce-Scatter operations for pipeline parallel processing | Same as FlashComm_v1, has threshold protection |
| Weight Prefetch | MLP-intensive scenarios (Dense models)| `export VLLM_ASCEND_ENABLE_PREFETCH_MLP=1` | Utilizes vector computation time to preload MLP weights into L2 Cache | Requires adjustment of prefetch buffer size |
| Async Scheduling | Large-scale models, high-concurrency scenarios | `--async-scheduling` | Non-blocking task scheduling, improving concurrent processing capability | Used in coordination with FullGraph optimization |
