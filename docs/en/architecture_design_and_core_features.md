...
- **Function**: Loads and launches compiled executable binaries.
...
Handles integer division and modulo operations in pointer expressions and mask expressions. By using dimensionality increase, it removes integer division and modulo operations and regenerates OPs like load/store.
...
| RewriteStoreOp | Analyzes the mask (`mask`) expression in the `tl.store` operation. Its function is similar to `RewriteLoadOp`, decomposing complex mask conditions containing integer division/modulo into a `MaskState` object. | Same as `RewriteLoadOp`. |
...
| RewriteAtomicCASOp | Handles pointer linearization issues in atomic compare-and-swap operations (`atomic.cas`). Analyzes its pointer expression and eliminates integer division and modulo operations through dimensionality increase to meet the addressing requirements of hardware atomic instructions. | |
...
| Converter Name | Description |
|---|---|
| BubbleUpExtract\<tensor::ExtractOp\> | Extract op upward movement optimization, which can avoid generating unnecessary loops in some scenarios |
| BubbleUpExtract\<tensor::ExtractSliceOp\> | Extract op/extract_slice upward movement optimization, which can avoid generating unnecessary loops in some scenarios |
...
| CatConverter | triton::CatOp to tensor::