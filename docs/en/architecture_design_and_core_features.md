...
| RewriteAtomicCASOp | Handles pointer linearization issues in atomic compare-and-swap operations (`atomic.cas`). Analyzes its pointer expression and eliminates integer division and modulo operations through dimension elevation to meet the addressing requirements of hardware atomic instructions. | |
