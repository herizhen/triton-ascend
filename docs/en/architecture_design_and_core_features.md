...
- **Purpose**: Used to express and optimize Ascend-specific computation graphs during the MLIR compilation process.
...
Handles integer division and remainder in pointer expressions and mask expressions. By using a dimension-raising method, it removes integer division and remainder and regenerates operations like load/store.
...
| RewriteStoreOp | Analyzes the mask expression in the `tl.store` operation. Its function is similar to `RewriteLoadOp`, decomposing complex mask conditions containing integer division/remainder into a `MaskState` object. | Same as `RewriteLoadOp`. |
...
| RewriteAtomicCASOp | Handles pointer linearization issues in atomic compare-and-swap operations (`atomic.cas`). Analyzes its pointer expression and eliminates integer division and remainder operations using a dimension-raising method to meet the addressing requirements of hardware atomic instructions. | |
...
| 3    | bubble-up-operation | Primarily performs an upward movement optimization for the order of `extract op/extract_slice`. This can optimize data locality and, in some scenarios, eliminate unnecessary loops generated after conversion, thereby improving the execution efficiency of the generated code. |
...
| Converter Name | Description |
|---|---|
| BubbleUpExtract\<tensor::ExtractOp\> | Upward movement optimization for extract op order, which can avoid generating unnecessary loops in some scenarios |
| BubbleUpExtract\<tensor::ExtractSliceOp\> | Upward movement optimization for extract op/extract_slice order, which can avoid generating unnecessary loops in some scenarios |
