...
Handles integer division and modulo operations in pointer expressions and mask expressions. By using a dimension-raising method, it removes integer division and modulo, then regenerates operations like load/store.
...
| RewriteAtomicCASOp | Handles pointer linearization issues in the atomic compare-and-swap operation (`atomic.cas`). Analyzes its pointer expression and eliminates integer division and modulo operations using dimension-raising methods to meet the addressing requirements of hardware atomic instructions. | |
...
| Converter Name | Description |
|---|---|
| BubbleUpExtract\<tensor::ExtractOp\> | Extract op order upward movement optimization, can avoid generating unnecessary loops in some scenarios |
| BubbleUpExtract\<tensor::ExtractSliceOp\> | Extract op/extract_slice order upward movement optimization, can avoid generating unnecessary loops in some scenarios |
