...
This project extends standard Triton to support Huawei Ascend NPU (via the CANN software stack). The overall design follows these **code principles**:
...
Handles integer division and modulo operations in pointer expressions and mask expressions. By using dimension elevation, it removes integer division and modulo operations and regenerates operations like load/store.
...
| 3 | bubble-up-operation | Mainly performs an upward movement optimization for the `extract op/extract_slice` sequence. This can optimize data locality and, in some scenarios, eliminate unnecessary loops generated after the conversion, thereby improving the execution efficiency of the generated code. |
