...
- **`compiler`**
  Receives the intermediate representation file `TTIR` (Triton IR) generated by the upper-layer Triton compiler and performs a series of transformations adapted for Ascend hardware.

  ```python
  Triton IR → Linalg IR → AscendNPU IR → triton_xxx_kernel.o
  ```

  Triton IR is converted to Linalg IR, and then via the BiSheng Compiler, an executable binary file `triton_xxx_kernel.o` targeting the Ascend NPU is generated.
...
This project extends the standard Triton to support Huawei's Ascend NPU (via the CANN software stack). The overall design follows these **code principles**:
...
- **Purpose**: Provides low-level implementation support adapted for Ascend NPU hardware, callable by Triton operators.
...
Handles integer division and modulo operations in pointer expressions and mask expressions. By using dimension elevation, it removes integer division and modulo operations and regenerates OPs such as load/store.

| Converter | Function | Limitations |
| ------------------------ | -------------------------- | ------------------------- |
| RewriteAddPtrOp | Analyzes pointer expressions (`AddPtrOp`) in operations like `tl.load`, `tl.store`. Decomposes the original pointer offset calculation and models it as a `PtrState` object containing specific offset information for each dimension (axis). For example, for an expression like `ptr + x // 1024 * 4096 + x % 1024 * 4 + y`, it analyzes the contribution and relationship of the `x` and `y` axes. | 1. The original iteration axis (e.g., `x`) must be divisible by the split axis (e.g., `1024`).<br>2. The external `XBLOCK` size must be an integer multiple or divisor of the split axis `divisor`. |
...
| RewriteLoadOp | Analyzes the mask (`mask`) expression in the `tl.load` operation. Decomposes complex mask conditions containing integer division/modulo and models them as a `MaskState` object containing boundary information for each dimension. For example, for `mask = x // 1024 < 8 and x % 1024 < 1024 and y < 4`, it analyzes the independent constraints for each dimension. | 1. The original iteration axis (e.g., `x`) must be divisible by the split axis (e.g., `1024`).<br>2. The external `XBLOCK` size must be an integer multiple or divisor of the split axis `divisor`. |
| BuildMask | Based on the analyzed `MaskState` object, reconstructs a new mask (`mask`) expression. The new mask will eliminate the integer division (`//`) and modulo (`%`) operations from the original expression. | Only processes `MaskState` generated by `RewriteLoadOp` or `RewriteStoreOp`. Cannot handle arbitrarily complex, non-canonical mask expressions. |
...
| RewriteStoreOp | Analyzes the mask (`mask`) expression in the `tl.store` operation. Its function is similar to `RewriteLoadOp`, decomposing complex mask conditions containing integer division/modulo and modeling them as a `MaskState` object. | Same as `RewriteLoadOp`. |
...
| RewriteAtomicRWMOp | Handles pointer issues in atomic read-write-modify operations (e.g., `atomic.add`, `atomic.max`, etc.). | Generally inherits the same limitations as `RewriteAddPtrOp`. May not support certain special, non-contiguous, or conditional atomic operation patterns. |
...
| Converter Name | Description |
|---|---|
| BubbleUpExtract\<tensor::ExtractOp\> | Upward movement optimization for extract op, which can avoid generating unnecessary loops in certain scenarios |
| BubbleUpExtract\<tensor::ExtractSliceOp\> | Upward movement optimization for extract op/extract_slice, which can avoid generating unnecessary loops in certain scenarios |
