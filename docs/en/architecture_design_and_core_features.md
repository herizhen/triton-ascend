...
- **`compiler`**
  ...
  Triton IR is converted to Linalg IR, which is then processed by the BiSheng Compiler to generate an executable binary file `triton_xxx_kernel.o` for the Ascend NPU.
...
> - **If a modification is target independent**, it should remain in the **Triton core** (e.g., general modifications to language, runtime);
> - **If a modification is strongly target affinitive** to Ascend hardware, it should be placed in **Triton-Ascend**.
...
Handles integer division and remainder operations in pointer expressions and mask expressions. By using a dimension-raising method, it removes integer division and remainder operations and regenerates OPs such as load/store.
...
| RewriteStoreOp | Analyzes the mask expression in the `tl.store` operation. Its function is similar to `RewriteLoadOp`, decomposing complex mask conditions containing integer division/remainder and modeling them into a `MaskState` object. | Same as `RewriteLoadOp`. |
...
| RewriteAtomicCASOp | Handles pointer linearization issues in the atomic compare-and-swap operation (`atomic.cas`). Analyzes its pointer expression and eliminates integer division and remainder operations using a dimension-raising method to meet the addressing requirements of hardware atomic instructions. | |
...
| 3    | bubble-up-operation | Primarily performs an upward movement optimization for `extract op/extract_slice` sequences. This can optimize data locality and, in some scenarios, eliminate unnecessary loops generated after the conversion, thereby improving the execution efficiency of the generated code. |
