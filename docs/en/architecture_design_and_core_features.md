...
Handles integer division and modulo operations in pointer expressions and mask expressions. By using dimension elevation, it removes integer division and modulo operations and regenerates OPs like load/store.

| Converter | Function | Limitations |
| ------------------------ | -------------------------- | ------------------------- |
| RewriteAddPtrOp | Analyzes pointer expressions (`AddPtrOp`) in operations like `tl.load`, `tl.store`. Decomposes the original pointer offset calculation and models it into a `PtrState` object containing specific offset information for each dimension (axis). For example, for an expression like `ptr + x // 1024 * 4096 + x % 1024 * 4 + y`, it analyzes the contributions and relationships of the `x` and `y` axes. | 1. The original iteration axis involved (e.g., `x`) must be divisible by the split axis (e.g., `1024`).<br>2. The outer `XBLOCK` size must be an integer multiple or divisor of the split axis `divisor`. |
...
| 3    | bubble-up-operation | Primarily performs upward movement optimization for `extract op/extract_slice` sequences. This can optimize data locality and, in some scenarios, eliminate unnecessary loops generated after conversion, thereby improving the execution efficiency of the generated code. |
...
| BitcastConverter | triton::Bitcast