...
- **Purpose**: Compiles high-level Triton DSL code into an **executable binary** (e.g., `.o` file) that can run on the Ascend NPU.
...
| :--- | :--------------------------------------------- | :--------------------------------- |
| 1    | `tl.insert_slice(full, src, offsets, sizes, strides)` | Inserts a tensor into another tensor according to the specified offsets, sizes, and strides parameters.<br>**Return value**: Target tensor.<br>**full**: Target tensor, the source tensor will be inserted into this tensor.<br>**src**: Source tensor.<br>**offsets**: Offsets on the target tensor (tuple of integers).<br>**sizes**: Sizes on the source tensor (tuple of integers).<br>**strides**: Strides on the target tensor (tuple of integers). |
| 2    | `tl.extract_slice(full, offsets, sizes, strides)` | Extracts a slice tensor from another tensor according to the specified offsets, sizes, and strides parameters.<br>**Return value**: Slice tensor.<br>**full**: Source tensor, the slice is extracted from this tensor.<br>**offsets**: Offsets on the source tensor (tuple of integers).<br>**sizes**: Sizes of the slice tensor (tuple of integers).<br>**strides**: Strides on the source tensor (tuple of integers). |
...
Handles integer division and modulo in pointer expressions and mask expressions. By using a dimension-raising method, it removes integer division and modulo and regenerates operations like load/store.

| Converter | Function | Limitations |
| ------------------------ | -------------------------- | ------------------------- |
| RewriteAddPtrOp | Analyzes pointer expressions (`AddPtrOp`) in operations like `tl.load`, `tl.store`. Decomposes the original pointer offset calculation and models it into a `PtrState` object containing specific offset information for each dimension (axis). For example, for an expression like `ptr + x // 1024 * 4096 + x % 1024 * 4 + y`, it analyzes the contributions and relationships of the `x` and `y` axes. | 1. The original iteration axis (e.g., `x`) must be divisible by the split axis (e.g., `1024`).<br>2. The external `XBLOCK` size must be an integer multiple or a divisor of the split axis `divisor`. |
...
| RewriteStoreOp | Analyzes the mask expression in the `tl.store` operation. Its function is similar to `RewriteLoadOp`, decomposing complex mask conditions containing integer division/modulo and modeling them into a `MaskState` object. | Same as `RewriteLoadOp`. |
...
| RewriteAtomicCASOp | Handles pointer linearization issues in atomic compare-and-swap operations (`atomic.cas`). Analyzes its pointer expression and eliminates integer division and modulo operations using a dimension-raising method to meet the addressing requirements of hardware atomic instructions. | |
...
|------|-------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1    | discrete-mask-access-conversion | Analyzes and converts memory access patterns based on discrete index masks (Discrete Mask) in Triton (e.g., `triton.language.load` with a non-contiguous mask), preparing for the subsequent expansion of discrete axes into loops. This Pass identifies irregular or sparse access patterns that cannot be efficiently handled by the backend hardware. |
...
| 3    | bubble-up-operation | Primarily performs an upward movement optimization for the `extract op/extract_slice` sequence. This can optimize data locality and, in some scenarios, eliminate unnecessary loops generated after the conversion, thereby improving the execution efficiency of the generated code. |

###### 3.2.2.2.1 discrete-mask-access-conversion

| Converter Name | Description|
|----------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| DiscreteMaskStoreConversion | First performs mask analysis. If the mask analysis result is non-contiguous, the original store operation is transformed into the following sequence:<br>1. load (load the content of the target storage address)<br>2. select (select between the target storage content and the value to be stored based on the mask)<br>3. store (store the result of the select operation back to the target address) |
| DiscreteMaskLoadConversion | First performs mask analysis. If the mask analysis result is non-contiguous, the original load operation is transformed into the following sequence:<br>1. load (load all content of the source tensor)<br>2. select (select content from the source tensor based on the mask, setting masked-out parts to the `other` value) |
| DiscreteMaskAtomicAddConversion | First performs mask analysis. If the mask analysis result is non-contiguous, the original atomic_add operation is transformed into the following sequence:<br>1. select (select the value based on the mask, setting masked-out parts to 0)<br>2. atomic_add (regenerate the atomic_add operation using the result after the select) |
...
| Converter Name | Description |
|---|---|
| BubbleUpExtract\<tensor::ExtractOp\> | extract op upward movement optimization, can avoid generating unnecessary loops in certain scenarios |
| BubbleUpExtract\<tensor::ExtractSliceOp\> | extract op/extract_slice upward movement optimization, can avoid generating unnecessary loops in certain scenarios |
