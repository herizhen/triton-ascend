...
- **`compiler`**
  ...
  Triton IR is converted to Linalg IR, and then the BiSheng Compiler generates an executable binary `triton_xxx_kernel.o` targeting the Ascend NPU.
...
Handles integer division and remainder operations in pointer expressions and mask expressions. By using dimension expansion, it removes integer division and remainder operations and regenerates OPs like load/store.
...
| RewriteStoreOp | Analyzes the mask expression in the `tl.store` operation. Its function is similar to `RewriteLoadOp`, decomposing complex mask conditions containing integer division/remainder and modeling them as a `MaskState` object. | Same as `RewriteLoadOp`. |
...
| RewriteAtomicCASOp | Handles pointer linearization issues in atomic compare-and-swap operations (`atomic.cas`). Analyzes its pointer expression and eliminates integer division and remainder operations using dimension expansion to meet the addressing requirements of hardware atomic instructions. | |
...
| 2    | triton-to-unstructured | Converts tensor operations containing discrete axes (Discrete Axes), identified by `discrete-mask-access-conversion`, into scalar memory access based on explicit scalar loops. |
...
| Converter Name | Description |
|---|---|
| BubbleUpExtract\<tensor::ExtractOp\> | Upward movement optimization for extract op, which can avoid generating unnecessary loops in some scenarios |
| BubbleUpExtract\<tensor::ExtractSliceOp\> | Upward movement optimization for extract op/extract_slice, which can avoid generating unnecessary loops in some scenarios |
