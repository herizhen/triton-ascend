...
- **Function**: Used to express and optimize Ascend-specific computation graphs within the MLIR compilation flow.

**`libdevice.py`**

- **Content**: `libdevice` interface adapted for the Ascend NPU.
- **Function**: Provides low-level implementation support adapted for Ascend NPU hardware, callable by Triton operators.

**`backend/compiler.py`**

- **Content**: Main entry point for the `triton-ascend` compiler.
- **Function**: Compiles high-level Triton DSL code into **executable binary files** (e.g., `.o` files) that can run on the Ascend NPU.

**`backend/driver.py`**

- **Content**: `triton-ascend` driver module.
- **Function**: Loads and launches the compiled executable binary.
...
| :--- | :--------------------------------------------- | :--------------------------------- |
| 1    | `tl.insert_slice(full, src, offsets, sizes, strides)` | Inserts a tensor into another tensor according to the specified offsets, sizes, and strides parameters.<br>**Return value**: Target tensor.<br>**full**: Target tensor, into which the source tensor will be inserted.<br>**src**: Source tensor.<br>**offsets**: Offsets on the target tensor (integer tuple).<br>**sizes**: Sizes on the source tensor (integer tuple).<br>**strides**: Strides on the target tensor (integer tuple). |
| 2    | `tl.extract_slice(full, offsets, sizes, strides)` | Extracts a slice tensor from another tensor according to the specified offsets, sizes, and strides parameters.<br>**Return value**: Slice tensor.<br>**full**: Source tensor, from which the slice is extracted.<br>**offsets**: Offsets on the source tensor (integer tuple).<br>**sizes**: Sizes of the slice tensor (integer tuple).<br>**strides**: Strides on the source tensor (integer tuple). |
| 3    | `tl.get_element(source, offset)` | Reads a tensor with dimensions and returns a single element at the specified offset.<br>**source**: Source tensor.<br>**offset**: Offset for the element extraction position (integer tuple). |

### 3.2 Triton-Ascend

#### 3.2.1 Compiler Options

| No. | NPUOptions | Hardware Platform | Usage |
| --- | --------------------------------------------- | ---------- | ----- |
...
Handles integer division and modulo in pointer expressions and mask expressions. By using dimension raising methods, it removes integer division and modulo operations and regenerates OPs like load/store.
...
| BuildMask | Based on the analyzed `MaskState` object, reconstructs a new mask expression. The new mask will eliminate the integer division (`//`) and modulo (`%`) operations from the original expression. | Only handles `MaskState` generated by `RewriteLoadOp` or `RewriteStoreOp`. Cannot handle arbitrarily complex, non-canonical mask expressions. |
...
| RewriteAtomicCASOp | Handles pointer linearization issues in atomic compare-and-swap operations (`atomic.cas`). Analyzes its pointer expression and eliminates integer division and modulo operations using dimension raising methods to meet the addressing requirements of hardware atomic instructions. | |
...
|------|-------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1    | discrete-mask-access-conversion | Analyzes and converts memory access patterns based on discrete index masks (Discrete Mask) in Triton (e.g., `triton.language.load` with non-contiguous mask), preparing for the subsequent expansion of discrete axes into loops. This Pass identifies irregular or sparse access patterns that cannot be efficiently handled by the backend hardware. |
...
| Converter Name | Description |
|---|---|
| BubbleUpExtract\<tensor::ExtractOp\> | Upward movement optimization for extract op, can avoid generating unnecessary loops in some scenarios |
| BubbleUpExtract\<tensor::ExtractSliceOp\> | Upward movement optimization for extract op/extract_slice, can avoid generating unnecessary loops in some scenarios |
...
| BitcastConverter | triton::BitcastOp to arith